...
def calculateMAE(pred):
    '''
    Calculates the mean average error and predict the error value
    '''
    diff= float()
    n = float()
    for values in pred:
...
lambda p: topMovieRecommendations(p[0], p[1], i.value, 5)).collect()

for r in user_movie_recs:
    print r
...
with open("ml-100k/u1.test") as f:
    ...
    # create train-test rating tuples and pass with actual data to get the error value
preds = []
for (user,movie_with_rating) in user_movie_recs:
        for (rating,movie) in movie_with_rating:
            for (test_item,test_rating) in test_ratings[user]:
                if str(test_item) == str(movie):
